...
// Ativa qualquer entidade (campanha, ad set ou anúncio) por ID — todas usam
// o mesmo endpoint POST /{id} {status: ACTIVE} na Graph API. Ao contrário
// das funções create*, esta liga gasto de verdade — só chamar com
// confirmação explícita do Patrick pra cada entidade especificamente.
async function activateEntity(entityId) {
  const url = `https://graph.facebook.com/${GRAPH_API_VERSION}/${entityId}`;
  const response = await fetch(url, {
    method: 'POST',
    headers: authHeaders({ 'Content-Type': 'application/json' }),
  ...
  const result = await response.json();
  if (result.error) {
    throw new Error(`Erro ao ativar entidade ${entityId}: ${JSON.stringify(result.error)}`);
  }
  return result;
}

// Estatísticas agregadas de eventos recebidos por um pixel/dataset (contagem
// por tipo de evento, ex: Purchase) — não precisa de ad_id, olha o dataset
// inteiro. Útil pra confirmar se o CAPI está de fato entregando eventos.
async function getDatasetStats(datasetId) {
  const url = `https://graph.facebook.com/${GRAPH_API_VERSION}/${datasetId}/stats?aggregation=event`;
  const response = await fetch(url, { headers: authHeaders() });
  const result = await response.json();
  if (result.error) {
    throw new Error(`Erro ao buscar stats do dataset ${datasetId}: ${JSON.stringify(result.error)}`);
  }
  return result;
}

async function getEntityStatus(entityId) {
  const url = `https://graph.facebook.com/${GRAPH_API_VERSION}/${entityId}?fields=status,effective_status,name`;
  const response = await fetch(url, { headers: authHeaders() });
  const result = await response.json();
  if (result.error) {
    throw new Error(`Erro ao buscar entidade ${entityId}: ${JSON.stringify(result.error)}`);
  }
  return result;
}
...
async function createAdSet(adAccountId, params) {
  ...
  // Sem isso, campanhas novas sem CBO exigem valor de lance mesmo pra
  // estratégia de menor custo — declarar explicitamente evita o erro.
  body.bid_strategy = params.bidStrategy || 'LOWEST_COST_WITHOUT_CAP';
...
async function createCampaign(adAccountId, params) {
  ...
  // Obrigatório desde a API v21: orçamento por ad set (ABO) exige declarar
  // explicitamente que não há compartilhamento de orçamento no nível da campanha.
  body.is_adset_budget_sharing_enabled = params.adSetBudgetSharingEnabled ?? false;
...
// Contas do Instagram que a CONTA DE ANÚNCIOS enxerga como autorizadas —
// diferente de getPageInstagramAccount, que olha a conexão da Página.
async function getAdAccountInstagramAccounts(adAccountId) {
  const url = `https://graph.facebook.com/${GRAPH_API_VERSION}/act_${adAccountId}/instagram_accounts`;
  const response = await fetch(url, { headers: authHeaders() });
  const result = await response.json();
  if (result.error) {
    throw new Error(`Erro ao buscar contas do Instagram da conta de anúncios ${adAccountId}: ${JSON.stringify(result.error)}`);
  }
  return result;
}
